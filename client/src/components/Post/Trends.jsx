...
const Trends = ({ posts, userData, usersData }) => {
  ...
  useEffect(() => {
    //a faire dans reducer et get le reducer
    return axios({
      method: "get",
      headers: { Authorization: "Bearer " + Cookies.get("jwt") },
      url:
        `${process.env.REACT_APP_API_URL}api/user/favorites-posts/` +
        userData._id,
    }).then((res) => {});
  });
    ...
    if (posts[0] !== undefined) {
      for (let i = 0; i < posts.length; i++) {
        if (posts[i].isEvent === true && incomingEvent === "") {
          ...
          posts[i].isEvent === true &&
          incomingEventDate !== undefined &&
  ...
  return (
    <>
      <div className="trends">
        ...
        {posts.map((posts) => {
          return incomingEvent === posts._id &&
            incomingEventDate === posts.date ? (
            <div className="eventBlock">
              <b>Prochain évènement</b>
              <div className="eventBlockText">
                <div className={"eventText" + posts.eventType}>
                  {posts.title}
                </div>
              ...
              {posts.picture && (
                <img
                  className={
                    "favoriteEventBanner favoriteEventBanner" + posts.eventType
                  }
                  src={process.env.REACT_APP_API_URL + posts.picture}
                />
              )}
              {posts.video && (
                <iframe
                  ...
                  <>
                    ...
                    <div className="modal-body">
                      <div className="modal-pic">
                        <img
                          src={
                            process.env.REACT_APP_API_URL + popupPost.picture
                          }
                        />
                      </div>
